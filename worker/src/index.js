// gifcompressors API — Cloudflare Worker entry.
// POST /v1/compress with a GIF, get a compressed GIF back. Files are processed
// in memory and never written to disk or logs.
...
export default {
  async fetch(request) {
    ...
    if (request.method === 'POST' && pathname === '/v1/compress') return handleCompress(request);
